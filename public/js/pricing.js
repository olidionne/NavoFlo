(() => {
  const currency = new Intl.NumberFormat(document.documentElement.lang.startsWith('fr') ? 'fr-CA' : 'en-CA', { style: 'currency', currency: 'CAD', maximumFractionDigits: 0 });
  const prices = {
  ...
  document.querySelectorAll('[data-plan-card]').forEach(card => {
    ...
    buy.addEventListener('click', async () => {
      ...
      buy.textContent = document.documentElement.lang.startsWith('fr') ? 'Ouverture de Stripe…' : 'Opening Stripe…';
      try {
        const response = await fetch('/api/stripe/create-checkout', {
          method: 'POST', headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ plan, seats: Number(input.value), locale: document.documentElement.lang.startsWith('fr') ? 'fr' : 'en' })
        });
        const data = await response.json();
        if (!response.ok || !data.url) throw new Error(data.error || 'Stripe unavailable');
        location.href = data.url;
      } catch (error) {
        alert((document.documentElement.lang.startsWith('fr') ? 'Paiement non configuré pour le moment : ' : 'Payment is not configured yet: ') + error.message);
        buy.disabled = false;
        buy.textContent = old;
      }
    });
