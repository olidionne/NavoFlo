/* NavoFlo V8.21.0 — deterministic fastener recognizer.
 *
...
export function fastenerNameHint(name){const s=String(name||'').replace(/[_.-]+/g,' ');for(const r of NAME_RULES)if(r.re.test(s))return{type:r.type,confidence:.985,source:'assembly-name'};return null;}
...
export function detectFastenerComponent({name='',geometry,faceInfo=[]}={}){
  const hint=fastenerNameHint(name),points=pointsOf(geometry),cyls=cylinderRecords(faceInfo);
  ...
  for(const h of throughHoles){
    const range=projection(points,h.a);if(!range?.span)continue;const outerR=radialEnvelope(points,h.a,h.c),orient=longitudinalPlaneOrientationCount(faceInfo,h.a),ratio=range.span/Math.max(outerR*2,EPS);
    if(outerR>h.r*1.55&&ratio<.80&&orient>=3){const conf=clamp(.88+(orient>=3?.04:0)+(hint?.type==='nut'?.07:0));if(conf>=.92||hint?.type==='nut')return{recognized:true,type:'nut',confidence:conf,source:hint?'name+brep':'brep-hex-through',diameterMm:outerR*2,lengthMm:range.span,evidence:['central-through-hole','polygonal-body','short-body'],name:String(name||'')};}
  }

  if(shank){
    const range=shank.range,outerR=radialEnvelope(points,shank.a,shank.c),diam=shank.r*2,headRatio=outerR/Math.max(shank.r,EPS),slender=range.span/Math.max(diam,EPS),orient=longitudinalPlaneOrientationCount(faceInfo,shank.a);
    // A standard bolt/screw has a long cylindrical shank plus a locally larger
    // head. A stud has no head but is accepted when metadata says stud/threaded rod.
    if(hint?.type==='stud'&&slender>=2.0)return{recognized:true,type:'stud',confidence:.985,source:'name+brep',diameterMm:diam,lengthMm:range.span,evidence:['cylindrical-shank','slender-body','name-prior'],name:String(name||'')};
    const hasHead=headRatio>=1.20&&(orient>=2||faceInfo.filter(f=>fam(f.family)==='plane').length>=4);
    if(hasHead&&slender>=1.15){
      const hinted=hint&&['bolt','screw'].includes(hint.type),type=hinted?hint.type:(blindHoles.length?'screw':'bolt'),conf=clamp(.88+(headRatio>=1.4?.035:0)+(orient>=3?.025:0)+(hinted?.07:0));
      if(conf>=.92||hinted)return{recognized:true,type,confidence:conf,source:hint?'name+brep':'brep-headed-shank',diameterMm:diam,lengthMm:range.span,evidence:['cylindrical-shank','head-envelope',orient>=3?'polygonal-head':'headed-body'],name:String(name||'')};
    }
  }
...
export const FASTENER_RECOGNIZER_VERSION='8.21.0';
