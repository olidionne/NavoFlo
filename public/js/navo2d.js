...
const state={
  ...
  trackingPoint:null,trackingPoints:[],trackingGuides:[],trackingSince:0,lastSnap:null,snapCycle:0,snapCandidateKey:'',
  ...
  offsetErase:false,offsetLayer:'source',
  textHeight:2.5,textRotation:0,mtextLineSpacing:1.0,mtextJustify:'TL',mtextWidth:40,textEditorDraft:null,
  rotationAngle:0,scaleFactor:1,
  dimTextRatio:0.020,dimArrowFactor:0.55,dimPrecision:4,dimArrowMode:'auto',dimLeaderLandingFactor:1.55,
...
function resetTransientDocumentState(){
  ...
  state.trackingPoint=null;state.trackingPoints=[];state.trackingGuides=[];state.lastSnap=null;state.snapCycle=0;state.gripEdit=null;state.hoverGrip=null;
  state.trimBrush=null;state.lastCrossingRect=null;
}
function restoreDocumentState(doc){
  closeTextEditor(false);closeMTextEditor(false);cancelGripEdit(false);cancelCommand(false);closeContextMenu();clearMeasure();
...
function navo2dPreferenceSnapshot(){
  return {
    ...
    mtextLineSpacing:Math.max(.5,Math.min(3,Number(state.mtextLineSpacing)||1)),mtextJustify:String(state.mtextJustify||'TL'),mtextWidth:Math.max(.000001,Number(state.mtextWidth)||40),
...
function applyNavo2DPreferences(p={}){
  ...
  state.mtextLineSpacing=finitePref(p.mtextLineSpacing,state.mtextLineSpacing,.5,3);state.mtextWidth=finitePref(p.mtextWidth,state.mtextWidth,1e-9,1e9);if(['TL','TC','TR','ML','MC','MR','BL','BC','BR'].includes(String(p.mtextJustify||'').toUpperCase()))state.mtextJustify=String(p.mtextJustify).toUpperCase();
...
function bindUI(){
  ...
  E.canvas.addEventListener('dblclick',event=>{
    if(state.command||state.gripEdit)return;
    const hit=hitTest(event.clientX,event.clientY);
    if(hit?.dimGenerated&&hit.dimGroupId){
      event.preventDefault();state.selected.clear();selectDimensionGroup(hit,false);syncSelectionUI();openDimensionEditor(hit.dimGroupId,event.clientX,event.clientY);return;
    }
    if(hit?.type==='TEXT'){
      event.preventDefault();state.selected=new Set([hit.id]);syncSelectionUI();openTextEditor(hit,event.clientX,event.clientY);
    }
  });
...
function canonicalize(raw,id){
  ...
  if((type==='TEXT'||type==='MTEXT')){
    ...
    if(pos){
      const value=String(raw.text??raw.string??'').replace(/\\P/g,'\n').replace(/[{}]/g,'');
      const isMText=type==='MTEXT';
      return {...base,type:'TEXT',rawType:type,point:p(pos),text:value,height:Math.abs(+raw.textHeight||+raw.height||2.5),rotation:+raw.rotation||0,
        mtext:isMText,width:isMText?Math.abs(+raw.width||+raw.referenceRectangleWidth||0):0,
        lineSpacing:isMText?Math.max(.5,Math.min(3,+raw.lineSpacingFactor||1)):1,justify:isMText?'TL':null};
    }
  }
  return null;
}
...
function wrapMTextModelLines(e){
  const text=String(e?.text??'').replace(/\r/g,'');
  if(!e?.mtext)return text.split('\n');
  const h=Math.max(Math.abs(Number(e.height)||state.textHeight||2.5),1e-9),width=Math.max(Math.abs(Number(e.width)||state.mtextWidth||h*8),h*1.2);
  const maxChars=Math.max(1,Math.floor(width/(h*.62))),out=[];
  for(const paragraph of text.split('\n')){
    if(!paragraph){out.push('');continue;}
    let line='';
    for(const word of paragraph.split(/\s+/)){
      const next=line?`${line} ${word}`:word;
      if(line&&next.length>maxChars){out.push(line);line=word;}else line=next;
    }
    out.push(line);
  }
  return out.length?out:[''];
}
function textWorldMetrics(e){
  const height=Math.max(Math.abs(Number(e?.height)||state.textHeight||2.5),1e-9);
  const text=String(e?.text??'');
  const lines=wrapMTextModelLines(e),lineSpacing=e?.mtext?Math.max(.5,Math.min(3,Number(e.lineSpacing)||1)):1;
  const width=e?.mtext?Math.max(Math.abs(Number(e.width)||state.mtextWidth||height*8),height*1.2):Math.max(height*.4,text.length*height*.62);
  const totalHeight=e?.mtext?Math.max(height,lines.length*height*lineSpacing):height;
  const a=(Number(e?.rotation)||0)*Math.PI/180,ux={x:Math.cos(a),y:Math.sin(a)},uy={x:-Math.sin(a),y:Math.cos(a)};
  return{height,width,totalHeight,lines,lineSpacing,ux,uy};
}
function textWorldCorners(e){
  const {width,totalHeight,ux,uy}=textWorldMetrics(e),o=e.point,sy=e?.mtext?-1:1;
  const at=(x,y)=>({x:o.x+ux.x*x+uy.x*y*sy,y:o.y+ux.y*x+uy.y*y*sy});
  return[at(0,0),at(width,0),at(width,totalHeight),at(0,totalHeight)];
}
function entitySamples(e){
  ...
  if(e.type==='TEXT')return textWorldCorners(e);if(e.type==='POINT')return[e.point];return[];
}
...
function drawEntity(e,color,width){
  ctx.strokeStyle=color;ctx.fillStyle=color;ctx.lineWidth=width;ctx.lineCap='round';ctx.lineJoin='round';ctx.setLineDash(e.centerMark?[10,4,2,4]:[]);
  if(e.type==='LINE'){const a=worldToScreen(e.p1),b=worldToScreen(e.p2);ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();ctx.setLineDash([]);return;}
  if(e.type==='POLYLINE'){const pts=e.points||[];if(!pts.length)return;ctx.beginPath();let s=worldToScreen(pts[0]);ctx.moveTo(s.x,s.y);for(let i=0;i<pts.length-1+(e.closed?1:0);i++){const a=pts[i%pts.length],b=pts[(i+1)%pts.length],bulge=a.bulge||0;if(Math.abs(bulge)<1e-10){s=worldToScreen(b);ctx.lineTo(s.x,s.y);}else{const arc=bulgeArc(a,b,bulge);for(const q of arc?.samples?.slice(1)||[]){s=worldToScreen(q);ctx.lineTo(s.x,s.y);}}}if(e.closed&&e.dimFilled){ctx.closePath();ctx.fill();}ctx.stroke();return;}
  ...
  if(e.type==='TEXT'){
    const sp=worldToScreen(e.point),m=textWorldMetrics(e),fontPx=Math.max(8,m.height*state.view.scale);
    ctx.save();ctx.translate(sp.x,sp.y);ctx.rotate(-e.rotation*Math.PI/180);ctx.font=`${fontPx}px ui-monospace,monospace`;ctx.textBaseline=e.mtext?'top':'alphabetic';
    if(e.mtext){
      const widthPx=m.width*state.view.scale,justify=String(e.justify||'TL').toUpperCase();
      ctx.textAlign=justify.includes('C')?'center':justify.includes('R')?'right':'left';
      const x=justify.includes('C')?widthPx/2:justify.includes('R')?widthPx:0;
      m.lines.forEach((line,i)=>ctx.fillText(line,x,i*fontPx*m.lineSpacing,widthPx));
    }else ctx.fillText(e.text,0,0);
    ctx.restore();ctx.setLineDash([]);
  }
}

// -----------------------------------------------------------------------------
// V6.7 — AutoCAD-like TEXT creation and editing.
// Double-click a TEXT entity or use TEXTEDIT / ED / DDEDIT to edit content,
// height, rotation and insertion coordinates without leaving the drawing.
// -----------------------------------------------------------------------------
let textEditorEl=null;
function closeTextEditor(commit=false){
  if(!textEditorEl){state.textEditorDraft=null;return;}
  if(commit&&state.textEditorDraft){
    const {entityId,draft}=state.textEditorDraft,entity=state.entities.find(e=>e.id===entityId);
    if(entity&&draft){
      pushHistory();
      entity.text=String(draft.text??'');entity.height=Math.max(1e-9,Math.abs(Number(draft.height)||state.textHeight||2.5));
      entity.rotation=Number(draft.rotation)||0;entity.point=copyPoint(draft.point||entity.point);
      state.textHeight=entity.height;state.textRotation=entity.rotation;afterGeometryChange();syncSelectionUI();
      toast(FR?'Texte modifié.':'Text updated.');
    }
  }
  textEditorEl.remove();textEditorEl=null;state.textEditorDraft=null;
  try{E.canvas.focus()}catch{}
}
function positionTextEditor(el,clientX,clientY){
  const margin=10,r=el.getBoundingClientRect();
  el.style.left=`${Math.max(margin,Math.min(clientX+12,innerWidth-r.width-margin))}px`;
  el.style.top=`${Math.max(margin,Math.min(clientY+12,innerHeight-r.height-margin))}px`;
}
function openTextEditor(entity,clientX=null,clientY=null){
  if(entity?.mtext){openMTextEditorForEntity(entity);return;}
  if(!entity||entity.type!=='TEXT')return commandError(FR?'Sélectionnez un texte.':'Select a text entity.');
  closeTextEditor(false);
  const draft=cloneEntity(entity,false);state.textEditorDraft={entityId:entity.id,draft};
  const panel=document.createElement('div');panel.className='n2-text-editor';
  panel.innerHTML=`<div class="n2-text-editor-head"><strong>${FR?'ÉDITER TEXTE':'EDIT TEXT'}</strong><button type="button" data-text-cancel>×</button></div>
    <label class="n2-text-editor-main"><span>${FR?'Texte':'Text'}</span><input type="text" data-text-value></label>
    <div class="n2-text-editor-grid">
      <label><span>${FR?'Hauteur':'Height'}</span><input type="number" step="any" min="0.000000001" data-text-height></label>
      <label><span>${FR?'Rotation':'Rotation'} °</span><input type="number" step="any" data-text-rotation></label>
      <label><span>X</span><input type="number" step="any" data-text-x></label>
      <label><span>Y</span><input type="number" step="any" data-text-y></label>
    </div>
    <div class="n2-text-editor-actions"><button type="button" data-text-cancel>${FR?'Annuler':'Cancel'}</button><button type="button" class="primary" data-text-save>${FR?'Appliquer':'Apply'}</button></div>`;
  document.body.append(panel);textEditorEl=panel;
  const value=panel.querySelector('[data-text-value]'),height=panel.querySelector('[data-text-height]'),rotation=panel.querySelector('[data-text-rotation]'),x=panel.querySelector('[data-text-x]'),y=panel.querySelector('[data-text-y]');
  value.value=draft.text||'';height.value=String(draft.height||state.textHeight);rotation.value=String(draft.rotation||0);x.value=String(draft.point.x);y.value=String(draft.point.y);
  const sync=()=>{draft.text=value.value;draft.height=Math.max(1e-9,Math.abs(parseNumber(height.value)||state.textHeight||2.5));draft.rotation=parseNumber(rotation.value)||0;const nx=parseNumber(x.value),ny=parseNumber(y.value);if(Number.isFinite(nx))draft.point.x=nx;if(Number.isFinite(ny))draft.point.y=ny;};
  [value,height,rotation,x,y].forEach(el=>el.addEventListener('input',sync));
  panel.querySelectorAll('[data-text-cancel]').forEach(b=>b.addEventListener('click',()=>closeTextEditor(false)));
  panel.querySelector('[data-text-save]').addEventListener('click',()=>{sync();closeTextEditor(true);});
  panel.addEventListener('keydown',event=>{if(event.key==='Escape'){event.preventDefault();closeTextEditor(false);}else if((event.ctrlKey||event.metaKey)&&event.key==='Enter'){event.preventDefault();sync();closeTextEditor(true);}});
  const sp=worldToScreen(entity.point),cr=E.canvas.getBoundingClientRect();positionTextEditor(panel,clientX??(cr.left+sp.x),clientY??(cr.top+sp.y));
  requestAnimationFrame(()=>{value.focus();value.select();});
}
function startTextEditPick(){
  if(state.selected.size===1){const e=state.entities.find(x=>state.selected.has(x.id));if(e?.type==='TEXT'){openTextEditor(e);return true;}}
  return false;
}
function commitTextEntity(c,text){
  const value=String(text??'');if(!value.length)return false;
  const e={...newEntityBase('TEXT'),point:copyPoint(c.data.point),text:value,height:Math.max(1e-9,Math.abs(Number(c.data.height)||state.textHeight||2.5)),rotation:Number(c.data.rotation)||0};
  addEntity(e);state.textHeight=e.height;state.textRotation=e.rotation;return true;
}

function mtextPromptOptions(){
  return FR?'[Hauteur/Justifier/Interligne/Rotation/Style/Largeur/Colonnes]':'[Height/Justify/LineSpacing/Rotation/Style/Width/Columns]';
}
function setMTextPrompt(c=state.command){
  if(!c)return;
  if(c.step==='firstCorner')setCommandPrompt(FR?`MTEXT Spécifiez le premier coin ou ${mtextPromptOptions()}:`:`MTEXT Specify first corner or ${mtextPromptOptions()}:`);
  else if(c.step==='oppositeCorner')setCommandPrompt(FR?`MTEXT Spécifiez le coin opposé ou ${mtextPromptOptions()}:`:`MTEXT Specify opposite corner or ${mtextPromptOptions()}:`);
}
function mtextReturnFromOption(c){
  c.step=c.data.returnStep||'firstCorner';delete c.data.returnStep;setMTextPrompt(c);
}
function enterMTextOption(c,key){
  c.data.returnStep=c.step;
  if(key==='H'){c.step='mtextHeight';setCommandPrompt(FR?`MTEXT Hauteur du texte <${fmt(c.data.height)}>:`:`MTEXT Text height <${fmt(c.data.height)}>:`);return true;}
  if(key==='J'){c.step='mtextJustify';setCommandPrompt(FR?`MTEXT Justification [HG/HC/HD/MG/MC/MD/BG/BC/BD] <${c.data.justify}>:`:`MTEXT Justification [TL/TC/TR/ML/MC/MR/BL/BC/BR] <${c.data.justify}>:`);return true;}
  if(key==='L'){c.step='mtextLineSpacing';setCommandPrompt(FR?`MTEXT Espacement des lignes <${fmt(c.data.lineSpacing)}>:`:`MTEXT Line spacing factor <${fmt(c.data.lineSpacing)}>:`);return true;}
  if(key==='R'){c.step='mtextRotation';setCommandPrompt(FR?`MTEXT Rotation <${fmt(c.data.rotation)}°>:`:`MTEXT Rotation <${fmt(c.data.rotation)}°>:`);return true;}
  if(key==='S'){c.step='mtextStyle';setCommandPrompt(FR?'MTEXT Style <STANDARD> (STANDARD seulement):':'MTEXT Style <STANDARD> (STANDARD only):');return true;}
  if(key==='W'){c.step='mtextWidth';setCommandPrompt(FR?`MTEXT Largeur <${fmt(c.data.width)}>:`:`MTEXT Width <${fmt(c.data.width)} >:`);return true;}
  if(key==='C'){c.step='mtextColumns';setCommandPrompt(FR?'MTEXT Colonnes <Aucune> (Aucune seulement):':'MTEXT Columns <None> (None only):');return true;}
  return false;
}
function normalizeMTextJustify(value){
  const u=String(value||'').trim().toUpperCase();
  const map={HG:'TL',HC:'TC',HD:'TR',MG:'ML',MC:'MC',MD:'MR',BG:'BL',BC:'BC',BD:'BR',TL:'TL',TC:'TC',TR:'TR',ML:'ML',MR:'MR',BL:'BL',BR:'BR'};
  return map[u]||null;
}
function mtextBoxFromCorners(cornerA,cornerB,data={}){
  const height=Math.max(Math.abs(Number(data.height)||state.textHeight||2.5),1e-9);
  const widthLocked=Boolean(data.widthLocked),signX=(cornerB.x>=cornerA.x)?1:-1;
  const width=Math.max(height*1.2,widthLocked?Math.abs(Number(data.width)||state.mtextWidth||40):Math.abs(cornerB.x-cornerA.x));
  const x=signX>=0?cornerA.x:cornerA.x-width;
  const top=Math.max(cornerA.y,cornerB.y),bottom=Math.min(cornerA.y,cornerB.y);
  const boxHeight=Math.max(height*1.8,top-bottom);
  return{origin:{x,y:top},width,boxHeight,bottom:top-boxHeight,right:x+width};
}
function commitMTextDraft(draft,entityId=null){
  const value=String(draft?.text??'');if(!value.trim())return false;
  const existing=entityId?state.entities.find(e=>e.id===entityId):null;
  if(existing){
    pushHistory();Object.assign(existing,draft,{id:existing.id,layer:existing.layer,type:'TEXT',mtext:true,rawType:'MTEXT'});afterGeometryChange();state.selected=new Set([existing.id]);syncSelectionUI();
  }else{
    const e={...newEntityBase('TEXT'),...draft,type:'TEXT',rawType:'MTEXT',mtext:true};addEntity(e);state.selected=new Set([e.id]);syncSelectionUI();
  }
  state.textHeight=Math.max(1e-9,Math.abs(Number(draft.height)||state.textHeight||2.5));
  state.textRotation=Number(draft.rotation)||0;state.mtextWidth=Math.max(1e-9,Math.abs(Number(draft.width)||state.mtextWidth||40));
  state.mtextLineSpacing=Math.max(.5,Math.min(3,Number(draft.lineSpacing)||1));state.mtextJustify=normalizeMTextJustify(draft.justify)||'TL';
  saveNavo2DPrefs();return true;
}
let mtextEditorEl=null;
function closeMTextEditor(commit=false){
  if(!mtextEditorEl)return;
  const stateData=mtextEditorEl._navoMText;
  if(commit&&stateData?.sync){stateData.sync();if(commitMTextDraft(stateData.draft,stateData.entityId))toast(FR?'Texte multiligne enregistré.':'Multiline text saved.');}
  mtextEditorEl.remove();mtextEditorEl=null;try{E.canvas.focus()}catch{}
}
function positionMTextEditor(panel,draft,boxHeight){
  const cr=E.canvas.getBoundingClientRect(),origin=worldToScreen(draft.point),w=Math.max(240,Math.min(620,draft.width*state.view.scale)),h=Math.max(120,Math.min(420,boxHeight*state.view.scale));
  panel.style.left=`${Math.max(cr.left+6,Math.min(cr.left+origin.x,innerWidth-w-8))}px`;
  panel.style.top=`${Math.max(cr.top+6,Math.min(cr.top+origin.y,innerHeight-h-8))}px`;
  panel.style.width=`${w}px`;panel.style.minHeight=`${h}px`;
  panel.style.transformOrigin='top left';panel.style.transform=`rotate(${-Number(draft.rotation||0)}deg)`;
}
function openMTextEditor(draft,boxHeight,entityId=null){
  closeTextEditor(false);closeMTextEditor(false);
  const panel=document.createElement('div');panel.className='n2-mtext-editor';
  panel.innerHTML=`<div class="n2-mtext-toolbar">
    <strong>MTEXT</strong>
    <label>H <input data-mt-height type="number" step="any" min="0.000000001"></label>
    <label>${FR?'Just.':'Just.'}<select data-mt-justify><option>TL</option><option>TC</option><option>TR</option><option>ML</option><option>MC</option><option>MR</option><option>BL</option><option>BC</option><option>BR</option></select></label>
    <label>${FR?'Interligne':'Spacing'} <input data-mt-spacing type="number" step=".05" min=".5" max="3"></label>
    <label>W <input data-mt-width type="number" step="any" min="0.000000001"></label>
    <label>° <input data-mt-rotation type="number" step="any"></label>
    <button type="button" data-mt-save title="Ctrl+Enter">✓</button><button type="button" data-mt-cancel>×</button>
  </div><textarea data-mt-value spellcheck="false"></textarea>`;
  document.body.append(panel);mtextEditorEl=panel;
  const value=panel.querySelector('[data-mt-value]'),height=panel.querySelector('[data-mt-height]'),justify=panel.querySelector('[data-mt-justify]'),spacing=panel.querySelector('[data-mt-spacing]'),width=panel.querySelector('[data-mt-width]'),rotation=panel.querySelector('[data-mt-rotation]');
  value.value=draft.text||'';height.value=String(draft.height||state.textHeight);justify.value=normalizeMTextJustify(draft.justify)||'TL';spacing.value=String(draft.lineSpacing||1);width.value=String(draft.width||state.mtextWidth);rotation.value=String(draft.rotation||0);
  const sync=()=>{draft.text=value.value;draft.height=Math.max(1e-9,Math.abs(parseNumber(height.value)||state.textHeight||2.5));draft.justify=normalizeMTextJustify(justify.value)||'TL';draft.lineSpacing=Math.max(.5,Math.min(3,parseNumber(spacing.value)||1));draft.width=Math.max(draft.height*1.2,Math.abs(parseNumber(width.value)||state.mtextWidth||40));draft.rotation=parseNumber(rotation.value)||0;value.style.textAlign=draft.justify.includes('C')?'center':draft.justify.includes('R')?'right':'left';value.style.lineHeight=String(draft.lineSpacing);positionMTextEditor(panel,draft,boxHeight);};
  panel._navoMText={draft,entityId,sync};
  [height,justify,spacing,width,rotation].forEach(el=>el.addEventListener('input',sync));
  panel.querySelector('[data-mt-save]').addEventListener('click',()=>closeMTextEditor(true));
  panel.querySelector('[data-mt-cancel]').addEventListener('click',()=>closeMTextEditor(false));
  panel.addEventListener('keydown',event=>{if(event.key==='Escape'){event.preventDefault();closeMTextEditor(false);}else if((event.ctrlKey||event.metaKey)&&event.key==='Enter'){event.preventDefault();closeMTextEditor(true);}});
  positionMTextEditor(panel,draft,boxHeight);requestAnimationFrame(()=>value.focus());
}
function openMTextEditorForEntity(entity){
  if(!entity?.mtext)return false;const metrics=textWorldMetrics(entity);openMTextEditor(cloneEntity(entity,false),metrics.totalHeight,entity.id);return true;
}


// -----------------------------------------------------------------------------
// V6.9 — semantic dimension groups.
// A dimension still exports as R12-safe LINE/ARC/TEXT/POLYLINE primitives, but
// every primitive carries the same dimMeta so Navo2D can treat it as one CAD
// object for selection, grips, text/leader movement and arrow flipping.
// -----------------------------------------------------------------------------
function dimensionGroupEntities(groupId){return state.entities.filter(e=>e.dimGenerated&&e.dimGroupId===groupId);}
function dimensionGroupMeta(groupId){return dimensionGroupEntities(groupId).find(e=>e.dimMeta)?.dimMeta||null;}
function dimensionGroupIdFromSelection(){const selected=state.entities.filter(e=>state.selected.has(e.id));if(!selected.length||selected.some(e=>!e.dimGenerated||!e.dimGroupId))return null;const ids=[...new Set(selected.map(e=>e.dimGroupId))];return ids.length===1?ids[0]:null;}
function selectDimensionGroup(entity,subtract=false){
  if(!entity?.dimGenerated||!entity.dimGroupId)return false;
  for(const e of dimensionGroupEntities(entity.dimGroupId)){if(subtract)state.selected.delete(e.id);else state.selected.add(e.id);}return true;
}
function expandDimensionSelection(){
  const gids=new Set(state.entities.filter(e=>state.selected.has(e.id)&&e.dimGenerated&&e.dimGroupId).map(e=>e.dimGroupId));
  for(const gid of gids)for(const e of dimensionGroupEntities(gid))state.selected.add(e.id);
}
function cycleDimensionArrowMode(groupId){
  const meta=dimensionGroupMeta(groupId);if(!meta)return false;
  const order=['auto','inside','outside'],i=order.indexOf(meta.arrowMode||'auto'),next=order[(i+1)%order.length];
...
function dimensionGroupGrips(groupId){
...
function selectedGrips(){
  ...
  const out=[],dimGroups=new Set();
  for(const e of state.entities)if(state.selected.has(e.id)){if(e.dimGenerated&&e.dimGroupId)dimGroups.add(e.dimGroupId);else out.push(...entityGrips(e));}
  for(const gid of dimGroups)out.push(...dimensionGroupGrips(gid));return out;
}
...
function beginGripEdit(grip){
  if(grip?.dimGroupId){
    const meta=dimensionGroupMeta(grip.dimGroupId);if(!meta)return false;
    state.gripEdit={entityId:null,dimGroupId:grip.dimGroupId,grip:{...grip,point:{...grip.point}},originalMeta:JSON.parse(JSON.stringify(meta)),base:{...grip.point},previewTarget:{...grip.point},dynField:'distance',dynDistance:null,dynAngle:null,armed:true};
    state.hoverGrip=grip;state.lastSnap=null;state.draftingGuide=null;closeContextMenu();if(E.commandInput){E.commandInput.value='';E.commandInput.placeholder=FR?'Déplacer la cote':'Move dimension';}return true;
  }
...
function resolveGripPoint(clientX,clientY){
  ...
  if(!state.lastSnap&&state.otrack&&state.trackingPoints.length)raw=applyObjectTracking(raw);
...
function dimensionMetaForGrip(meta,grip,target){const m=JSON.parse(JSON.stringify(meta));if(grip.role==='dimLocation'||grip.role==='dimText')m.location=copyPoint(target);return m;}
function commitGripEdit(target){
  const g=state.gripEdit;if(!g)return false;const finalTarget=target||g.previewTarget||g.base;if(dist(finalTarget,g.base)<1e-12){cancelGripEdit();return false;}
  if(g.dimGroupId){const meta=dimensionMetaForGrip(g.originalMeta,g.grip,finalTarget);pushHistory();rebuildDimensionGroup(g.dimGroupId,meta);afterGeometryChange();state.selected=new Set(dimensionGroupEntities(g.dimGroupId).map(e=>e.id));syncSelectionUI();commandLog(`DIM GRIP · ${fmt(finalTarget.x)}, ${fmt(finalTarget.y)}`);cancelGripEdit();return true;}
  const entity=state.entities.find(e=>e.id===g.entityId);if(!entity){cancelGripEdit();return false;}
...
function previewGripEntity(){
  const g=state.gripEdit;if(!g||g.dimGroupId)return null;const e=cloneEntity(g.original,false),target=gripTargetFromDynamic()||g.previewTarget||g.base;applyGripMutation(e,g.grip,target);return e;
}
function drawGripPreview(){
  const g=state.gripEdit;if(!g)return;
  if(g.dimGroupId){const target=gripTargetFromDynamic()||g.previewTarget||g.base,meta=dimensionMetaForGrip(g.originalMeta,g.grip,target);ctx.save();ctx.globalAlpha=.92;ctx.setLineDash([6,4]);drawPreviewEntities(buildDimensionFromMeta(meta,'preview-grip'));ctx.restore();return;}
  const e=previewGripEntity();if(!e)return;ctx.save();ctx.globalAlpha=.92;ctx.setLineDash([6,4]);drawEntity(e,'#7bc4ff',1.45);ctx.setLineDash([]);ctx.restore();
}
...
function pointerUp(ev){
  ...
  if(d.mode==='selection-box'){
    ...
    expandDimensionSelection();syncSelectionUI();
    return;
  }
  ...
  if(state.command?.selecting||(!state.command&&state.tool==='select')){
    ...
    if(hit){
      // AutoCAD selection sets accumulate. A generated dimension behaves as one object.
      if(hit.dimGenerated&&hit.dimGroupId)selectDimensionGroup(hit,Boolean(ev.shiftKey));
      else if(ev.shiftKey)state.selected.delete(hit.id);
...
function selectByBox(box,subtract=false){
  ...
  for(const id of matches){const e=state.entities.find(x=>x.id===id);if(e?.dimGenerated&&e.dimGroupId)dimGroups.add(e.dimGroupId);else if(subtract)state.selected.delete(id);else state.selected.add(id);}
  for(const gid of dimGroups)for(const e of dimensionGroupEntities(gid)){if(subtract)state.selected.delete(e.id);else state.selected.add(e.id);}
...
function selectionSegments(entity,samples){
  ...
  if(entity.type==='CIRCLE'||entity.type==='TEXT'||(entity.type==='POLYLINE'&&entity.closed))out.push([samples[samples.length-1],samples[0]]);
  return out;
}
...
function distanceToEntity(q,e){
  ...
  if(e.type==='TEXT'){
    const {height,width,ux,uy}=textWorldMetrics(e),dx=q.x-e.point.x,dy=q.y-e.point.y;
    const lx=dx*ux.x+dy*ux.y,ly=dx*uy.x+dy*uy.y;
    const cx=clamp(lx,0,width),cy=clamp(ly,0,height);
    return Math.hypot(lx-cx,ly-cy);
  }
  if(e.type==='POINT')return dist(q,e.point);return Infinity;
}
...
function syncSelectionUI(){
  if(state.gripEdit&&state.gripEdit.entityId&&!state.selected.has(state.gripEdit.entityId))cancelGripEdit(false);
  E.del.disabled=!state.selected.size;E.move.disabled=!state.selected.size;const sel=state.entities.filter(e=>state.selected.has(e.id)),dimGid=dimensionGroupIdFromSelection();if(!sel.length)E.selectionInfo.textContent=T.noSelection;else if(dimGid){const m=dimensionGroupMeta(dimGid);E.selectionInfo.textContent=`DIMENSION\n${m?.kind||'dimension'}\n${FR?'Flèches':'Arrows'}: ${m?.arrowMode||'auto'}`;}else if(sel.length===1){const e=sel[0];let s=`${e.rawType||e.type}\nLayer: ${e.layer}`;if(e.type==='LINE')s+=`\nLength: ${formatLength(dist(e.p1,e.p2))}`;if(e.type==='CIRCLE')s+=`\nØ ${formatLength(e.radius*2)}`;if(e.type==='ARC')s+=`\nR ${formatLength(e.radius)}`;if(e.type==='TEXT')s+=`\n${FR?'Texte':'Text'}: ${e.text}\n${FR?'Hauteur':'Height'}: ${formatLength(e.height)}\n${FR?'Rotation':'Rotation'}: ${fmt(e.rotation||0)}°`;if(e.approx)s+=`\n≈ ${FR?'géométrie approximée':'approximated geometry'}`;E.selectionInfo.textContent=s;}else E.selectionInfo.textContent=`${sel.length} ${T.selected}`;
  syncLayerPicker();
}
...
function writeEntity(add,e){
  ...
  if(e.type==='TEXT'){
    if(e.mtext){
      const m=textWorldMetrics(e),a=(Number(e.rotation)||0)*Math.PI/180,down={x:Math.sin(a),y:-Math.cos(a)};
      for(let i=0;i<m.lines.length;i++){
        const q={x:e.point.x+down.x*i*m.height*m.lineSpacing,y:e.point.y+down.y*i*m.height*m.lineSpacing};
        add(0,'TEXT');common();add(10,q.x);add(20,q.y);add(30,0);add(40,m.height);add(1,safeDxfText(m.lines[i]));add(50,Number(e.rotation)||0);add(7,'STANDARD');
      }
      return true;
    }
...
const COMMAND_ALIASES=new Map(Object.entries({
  ...
  T:'TEXT',TEXT:'TEXT',DT:'TEXT',DTEXT:'TEXT',MT:'MTEXT',MTEXT:'MTEXT',ED:'TEXTEDIT',DDEDIT:'TEXTEDIT',TEXTEDIT:'TEXTEDIT',
...
function handleCommandInputKeyDown(event){
  ...
  if(event.key===' '&&state.command?.name==='TEXT'&&state.command.step==='content'){return;}
  if(event.key==='Enter'||event.key===' '){
...
function startCommand(rawName){
  closeTextEditor(false);closeMTextEditor(false);
  state.trackingPoint=null;state.trackingPoints=[];state.trackingGuides=[];
  resetDynamicInput();
  ...
  if(name==='TEXTEDIT'){
    if(startTextEditPick()){state.lastCommand='TEXTEDIT';return;}
    if(!state.file){toast(FR?'Ouvrez un DXF ou tapez NEW.':'Open a DXF or type NEW.');return;}
  }

  const drawingCommands=new Set(['LINE','PLINE','CIRCLE','ARC','RECTANG','TEXT','MTEXT']);
  ...
  else if(name==='TEXT'){state.command.step='insertion';Object.assign(state.command.data,{height:state.textHeight,rotation:state.textRotation});setCommandPrompt(FR?'TEXT Spécifiez le point d’insertion:':'TEXT Specify insertion point:');}
  else if(name==='MTEXT'){state.command.step='firstCorner';Object.assign(state.command.data,{height:state.textHeight,rotation:state.textRotation,justify:state.mtextJustify,lineSpacing:state.mtextLineSpacing,width:state.mtextWidth,style:'STANDARD',columns:'NONE'});setMTextPrompt(state.command);}
  else if(name==='TEXTEDIT'){state.command.step='selectEntity';setCommandPrompt(FR?'TEXTEDIT Sélectionnez le texte à modifier:':'TEXTEDIT Select text to edit:');}
  ...
  else if(name==='OFFSET'){state.command.step='distance';Object.assign(state.command.data,{erase:state.offsetErase,layerMode:state.offsetLayer,lastCreatedId:null,lastSource:null});setCommandPrompt(FR?`${name} Distance de décalage ou [ÀTravers/Effacer/Layer]:`:`${name} Offset distance or [Through/Erase/Layer]:`);}
...
function afterSelectionConfirmed(name){
  ...
  if(name==='MOVE'||name==='COPY'||name==='ROTATE'||name==='SCALE'||name==='STRETCH'){
    ...
    if(name==='SCALE')Object.assign(state.command.data,{copy:false,referenceLength:null,refP1:null,newLengthP1:null});
...
function commandEnter(){
  ...
  if(c.name==='MTEXT'){
    if(c.step==='mtextHeight'){c.data.height=state.textHeight;mtextReturnFromOption(c);return;}
    if(c.step==='mtextJustify'){c.data.justify=state.mtextJustify;mtextReturnFromOption(c);return;}
    if(c.step==='mtextLineSpacing'){c.data.lineSpacing=state.mtextLineSpacing;mtextReturnFromOption(c);return;}
    if(c.step==='mtextRotation'){c.data.rotation=state.textRotation;mtextReturnFromOption(c);return;}
    if(c.step==='mtextWidth'){c.data.width=state.mtextWidth;c.data.widthLocked=true;mtextReturnFromOption(c);return;}
    if(c.step==='mtextStyle'){c.data.style='STANDARD';mtextReturnFromOption(c);return;}
    if(c.step==='mtextColumns'){c.data.columns='NONE';mtextReturnFromOption(c);return;}
    cancelCommand(true);return;
  }
  if(c.name==='ROTATE'){
    if(c.step==='angle'){
  ...
  if(c.name==='COPY'&&c.step==='target'){if(c.data.lastCopyIds?.length)state.selected=new Set(c.data.lastCopyIds);finishCommand();return;}
  if(c.name==='MIRROR'&&c.step==='eraseSource'){
    performMirror(false);finishCommand();return;
  }
  if(c.name==='OFFSET'){
    if(c.step==='eraseMode'){c.data.erase=state.offsetErase;c.step='distance';setCommandPrompt(FR?`${c.name} Distance de décalage ou [ÀTravers/Effacer/Layer]:`:`${c.name} Offset distance or [Through/Erase/Layer]:`);return;}
    if(c.step==='layerMode'){c.data.layerMode=state.offsetLayer;c.step='distance';setCommandPrompt(FR?`${c.name} Distance de décalage ou [ÀTravers/Effacer/Layer]:`:`${c.name} Offset distance or [Through/Erase/Layer]:`);return;}
...
function commandText(raw){
  ...
  if(c.name==='MTEXT'){
    if(['firstCorner','oppositeCorner'].includes(c.step)){
      const aliases=FR
        ?{H:'H',HEIGHT:'H',HAUTEUR:'H',J:'J',JUSTIFY:'J',JUSTIFIER:'J',I:'L',INTERLIGNE:'L',L:'W',LARGEUR:'W',R:'R',ROTATION:'R',S:'S',STYLE:'S',W:'W',WIDTH:'W',C:'C',COLUMNS:'C',COLONNES:'C'}
        :{H:'H',HEIGHT:'H',J:'J',JUSTIFY:'J',L:'L',LINESPACING:'L',R:'R',ROTATION:'R',S:'S',STYLE:'S',W:'W',WIDTH:'W',C:'C',COLUMNS:'C'};
      const key=aliases[u];if(key&&enterMTextOption(c,key))return;
    }
    if(c.step==='mtextHeight'){const n=parseNumber(raw);if(!(n>0))return commandError(CMDT.valueInvalid);c.data.height=state.textHeight=n;mtextReturnFromOption(c);return;}
    if(c.step==='mtextJustify'){const j=normalizeMTextJustify(raw);if(!j)return commandError(CMDT.valueInvalid);c.data.justify=state.mtextJustify=j;mtextReturnFromOption(c);return;}
    if(c.step==='mtextLineSpacing'){const n=parseNumber(raw);if(!(n>=.5&&n<=3))return commandError(CMDT.valueInvalid);c.data.lineSpacing=state.mtextLineSpacing=n;mtextReturnFromOption(c);return;}
    if(c.step==='mtextRotation'){const n=parseNumber(raw);if(!Number.isFinite(n))return commandError(CMDT.valueInvalid);c.data.rotation=state.textRotation=n;mtextReturnFromOption(c);return;}
    if(c.step==='mtextWidth'){const n=parseNumber(raw);if(!(n>0))return commandError(CMDT.valueInvalid);c.data.width=state.mtextWidth=n;c.data.widthLocked=true;mtextReturnFromOption(c);return;}
    if(c.step==='mtextStyle'){if(u&&u!=='STANDARD')return commandError(FR?'Seul le style STANDARD est pris en charge pour le moment.':'Only STANDARD style is currently supported.');c.data.style='STANDARD';mtextReturnFromOption(c);return;}
    if(c.step==='mtextColumns'){if(u&&!['NONE','AUCUNE','AUCUN','0'].includes(u))return commandError(FR?'Les colonnes MTEXT ne sont pas encore prises en charge.':'MTEXT columns are not supported yet.');c.data.columns='NONE';mtextReturnFromOption(c);return;}
  }

  if(c.name==='TEXT'){
    if(c.step==='height'){const n=parseNumber(raw);if(!(n>0))return commandError(FR?'La hauteur doit être supérieure à 0.':'Height must be greater than 0.');c.data.height=n;state.textHeight=n;c.step='rotation';setCommandPrompt(FR?`TEXT Angle de rotation <${fmt(state.textRotation)}°>:`:`TEXT Rotation angle <${fmt(state.textRotation)}°>:`);return;}
    if(c.step==='rotation'){const n=parseNumber(raw);if(!Number.isFinite(n))return commandError(CMDT.valueInvalid);c.data.rotation=n;state.textRotation=n;c.step='content';setCommandPrompt(FR?'TEXT Entrez le texte:':'TEXT Enter text:');return;}
    if(c.step==='content'){if(commitTextEntity(c,String(raw))){finishCommand();}return;}
  }
  ...
  if(c.name==='COPY'&&c.step==='target'){
    if(u==='E'||u==='EXIT'||u==='Q'||u==='QUITTER'){if(c.data.lastCopyIds?.length)state.selected=new Set(c.data.lastCopyIds);finishCommand();return;}
    if(u==='U'||u==='UNDO'||u==='A'||u==='ANNULER'){const batch=c.data.copyBatches?.pop();if(batch?.length){pushHistory();const ids=new Set(batch);state.entities=state.entities.filter(e=>!ids.has(e.id));c.data.lastCopyIds=c.data.copyBatches?.at(-1)||[];afterGeometryChange();state.selected=new Set(c.data.sourceIds||[]);syncSelectionUI();toast(FR?'Dernière copie annulée.':'Last copy undone.');}return;}
  }

  if(c.name==='OFFSET'){
    if(c.step==='distance'){
      if(['T','THROUGH','A','ÀTRAVERS'].includes(u)){c.data.through=true;c.step='selectEntity';setCommandPrompt(FR?'OFFSET Sélectionnez l’objet à décaler ou [Quitter/Annuler] <Quitter>:' :'OFFSET Select object to offset or [Exit/Undo] <Exit>:');return;}
      if(['E','ERASE','EFFACER'].includes(u)){c.step='eraseMode';setCommandPrompt(FR?`OFFSET Effacer l’objet source ? [Oui/Non] <${state.offsetErase?'Oui':'Non'}>:`:`OFFSET Erase source object? [Yes/No] <${state.offsetErase?'Yes':'No'}>:`);return;}
      if(['L','LAYER','CALQUE'].includes(u)){c.step='layerMode';setCommandPrompt(FR?`OFFSET Layer de destination [Courant/Source] <${state.offsetLayer==='current'?'Courant':'Source'}>:`:`OFFSET Destination layer [Current/Source] <${state.offsetLayer==='current'?'Current':'Source'}>:`);return;}
      const n=parseNumber(raw);if(!(n>0))return commandError(CMDT.valueInvalid);c.data.distance=n;c.data.through=false;c.step='selectEntity';setCommandPrompt(FR?'OFFSET Sélectionnez l’objet à décaler ou [Quitter/Annuler] <Quitter>:' :'OFFSET Select object to offset or [Exit/Undo] <Exit>:');return;
    }
    if(c.step==='eraseMode'){if(['Y','YES','O','OUI'].includes(u))state.offsetErase=c.data.erase=true;else if(['N','NO','NON'].includes(u))state.offsetErase=c.data.erase=false;else return commandError(CMDT.valueInvalid);c.step='distance';setCommandPrompt(FR?`${c.name} Distance de décalage ou [ÀTravers/Effacer/Layer]:`:`${c.name} Offset distance or [Through/Erase/Layer]:`);return;}
    if(c.step==='layerMode'){if(['C','CURRENT','COURANT'].includes(u))state.offsetLayer=c.data.layerMode='current';else if(['S','SOURCE'].includes(u))state.offsetLayer=c.data.layerMode='source';else return commandError(CMDT.valueInvalid);c.step='distance';setCommandPrompt(FR?`${c.name} Distance de décalage ou [ÀTravers/Effacer/Layer]:`:`${c.name} Offset distance or [Through/Erase/Layer]:`);return;}
    if(c.step==='selectEntity'){
      if(['E','EXIT','Q','QUITTER'].includes(u)){finishCommand();return;}
      if(['U','UNDO','A','ANNULER'].includes(u)&&c.data.lastCreatedId){pushHistory();state.entities=state.entities.filter(e=>e.id!==c.data.lastCreatedId);if(c.data.lastSource)state.entities.push(c.data.lastSource);c.data.lastCreatedId=null;c.data.lastSource=null;afterGeometryChange();return;}
    }
  }
  if(c.name==='ROTATE'){
    if(c.step==='angle'){
  ...
  if(c.name==='SCALE'){
    if(c.step==='factor'){
      if(u==='C'||u==='COPY'||u==='COPIE'){c.data.copy=true;setCommandPrompt(FR?`SCALE Spécifiez le facteur d’échelle ou [Référence] <${fmt(state.scaleFactor)} >:`:`SCALE Specify scale factor or [Reference] <${fmt(state.scaleFactor)} >:`);return;}
      if(u==='R'||u==='REFERENCE'||u==='RÉFÉRENCE'){c.step='referenceLength';c.data.referenceLength=null;c.data.refP1=null;setCommandPrompt(FR?'SCALE Spécifiez la longueur de référence <1>:':'SCALE Specify reference length <1>:');return;}
      const n=parseNumber(raw);if(!(n>0))return commandError(CMDT.valueInvalid);state.scaleFactor=n;performScale(n,c.data.copy===true);finishCommand();return;
    }
    if(c.step==='referenceLength'){
      const n=parseNumber(raw);if(!(n>0))return commandError(CMDT.valueInvalid);c.data.referenceLength=n;c.step='newLength';setCommandPrompt(FR?`SCALE Spécifiez la nouvelle longueur ou [Points] <${fmt(n)} >:`:`SCALE Specify new length or [Points] <${fmt(n)} >:`);return;
    }
    if(c.step==='newLength'){
      if(u==='P'||u==='POINTS'){c.step='newLengthPoint1';setCommandPrompt(FR?'SCALE Spécifiez le premier point de la nouvelle longueur:':'SCALE Specify first point of new length:');return;}
      const n=parseNumber(raw),ref=Number(c.data.referenceLength)||1;if(!(n>0)||!(ref>0))return commandError(CMDT.valueInvalid);const factor=n/ref;state.scaleFactor=factor;performScale(factor,c.data.copy===true);finishCommand();return;
    }
  }
...
function commandPoint(point,event,fromKeyboard=false){
  ...
  if(c.name==='TEXT'){
    if(c.step==='insertion'){c.data.point=copyPoint(point);c.step='height';setCommandPrompt(FR?`TEXT Hauteur <${fmt(state.textHeight)} >:`:`TEXT Height <${fmt(state.textHeight)} >:`);return;}
  }
  if(c.name==='MTEXT'){
    if(c.step==='firstCorner'){c.data.firstCorner=copyPoint(point);c.step='oppositeCorner';setMTextPrompt(c);return;}
    if(c.step==='oppositeCorner'){
      const box=mtextBoxFromCorners(c.data.firstCorner,point,c.data);
      if(!(box.width>1e-9&&box.boxHeight>1e-9))return commandError(CMDT.pointInvalid);
      const draft={point:box.origin,text:'',height:Math.max(1e-9,Math.abs(Number(c.data.height)||state.textHeight||2.5)),rotation:Number(c.data.rotation)||0,mtext:true,width:box.width,lineSpacing:Number(c.data.lineSpacing)||1,justify:normalizeMTextJustify(c.data.justify)||'TL',style:'STANDARD',columns:'NONE'};
      finishCommand(false);openMTextEditor(draft,box.boxHeight,null);return;
    }
  }
  if(c.name==='TEXTEDIT'&&c.step==='selectEntity'){
  ...
  if(c.name==='DIM'){
    ...
    if(c.step==='place'){
      const items=c.data.dimType==='linear'?buildLinearDimension(c.data.p1,c.data.p2,point,'auto'):buildAlignedDimension(c.data.p1,c.data.p2,point);
      if(commitDimension(items,c.data.dimType==='linear'?{kind:'linear',p1:copyPoint(c.data.p1),p2:copyPoint(c.data.p2),location:copyPoint(point),orientation:'auto',arrowMode:state.dimArrowMode}:{kind:'aligned',p1:copyPoint(c.data.p1),p2:copyPoint(c.data.p2),location:copyPoint(point),arrowMode:state.dimArrowMode})){finishCommand();}return;
    }
    if(c.step==='curvePlace'){
      ...
      if(commitDimension(items,{kind:c.data.dimType,sourceEntityId:e.id,location:copyPoint(point),arrowMode:state.dimArrowMode})){finishCommand();}return;
    }
  }
  if(c.name==='DIMLINEAR'||c.name==='DIMALIGNED'){
    ...
    if(c.step==='place'){
      ...
      if(commitDimension(items,c.name==='DIMLINEAR'?{kind:'linear',p1:copyPoint(c.data.p1),p2:copyPoint(c.data.p2),location:copyPoint(point),orientation:'auto',arrowMode:state.dimArrowMode}:{kind:'aligned',p1:copyPoint(c.data.p1),p2:copyPoint(c.data.p2),location:copyPoint(point),arrowMode:state.dimArrowMode})){finishCommand();}return;
    }
  }
  if(c.name==='DIMRADIUS'||c.name==='DIMDIAMETER'){
    ...
    if(c.step==='place'){
      ...
      if(commitDimension(items,{kind:c.name==='DIMRADIUS'?'radius':'diameter',sourceEntityId:e.id,location:copyPoint(point),arrowMode:state.dimArrowMode})){finishCommand();}return;
    }
  }
  ...
  if(c.name==='SCALE'){
    if(c.step==='referenceLength'){c.data.refP1=copyPoint(point);c.step='referenceSecond';setCommandPrompt(FR?'SCALE Spécifiez le deuxième point de la longueur de référence:':'SCALE Specify second point of reference length:');return;}
    if(c.step==='referenceSecond'){if(!c.data.refP1||dist(c.data.refP1,point)<1e-12)return commandError(CMDT.pointInvalid);c.data.referenceLength=dist(c.data.refP1,point);c.step='newLength';setCommandPrompt(FR?`SCALE Spécifiez la nouvelle longueur ou [Points] <${fmt(c.data.referenceLength)} >:`:`SCALE Specify new length or [Points] <${fmt(c.data.referenceLength)} >:`);return;}
    if(c.step==='newLength'){const ref=Number(c.data.referenceLength)||1,n=dist(c.data.base,point);if(!(n>0)||!(ref>0))return commandError(CMDT.valueInvalid);const factor=n/ref;state.scaleFactor=factor;performScale(factor,c.data.copy===true);finishCommand();return;}
    if(c.step==='newLengthPoint1'){c.data.newLengthP1=copyPoint(point);c.step='newLengthPointSecond';setCommandPrompt(FR?'SCALE Spécifiez le deuxième point de la nouvelle longueur:':'SCALE Specify second point of new length:');return;}
    if(c.step==='newLengthPointSecond'){if(!c.data.newLengthP1||dist(c.data.newLengthP1,point)<1e-12)return commandError(CMDT.pointInvalid);const ref=Number(c.data.referenceLength)||1,n=dist(c.data.newLengthP1,point),factor=n/ref;state.scaleFactor=factor;performScale(factor,c.data.copy===true);finishCommand();return;}
  }
  ...
  if(c.name==='MOVE'||c.name==='COPY'||c.name==='ROTATE'||c.name==='SCALE'){
    if(c.step==='base'){
      ...
      else if(c.name==='SCALE')setCommandPrompt(FR?`SCALE Spécifiez le facteur d’échelle ou [Copie/Référence] <${fmt(state.scaleFactor)} >:`:`SCALE Specify scale factor or [Copy/Reference] <${fmt(state.scaleFactor)} >:`);
      else setCommandPrompt(`${c.name} ${CMDT.secondPoint}:`);
      return;
    }
    if(c.step==='target'){
      const dx=point.x-c.data.base.x,dy=point.y-c.data.base.y;
      if(c.name==='MOVE'){pushHistory();translateSelected(dx,dy);toast(T.moved);finishCommand();return;}
      const copies=performCopy(dx,dy,c.data.sourceIds);c.data.lastCopyIds=copies.map(e=>e.id);c.data.copyBatches.push([...c.data.lastCopyIds]);state.selected=new Set(c.data.sourceIds);syncSelectionUI();setCommandPrompt(FR?'COPY Spécifiez le point suivant ou [Quitter/Annuler] <Quitter>:' :'COPY Specify next point or [Exit/Undo] <Exit>:');return;
    }
    ...
    if(c.step==='factor'){
      ...
      if(factor>1e-12){state.scaleFactor=factor;performScale(factor,c.data.copy===true);finishCommand();}return;
    }
  }
  if(c.name==='MIRROR'){
  ...
  if(c.name==='OFFSET'){
    ...
    if(c.step==='side'){
      ...
      pushHistory();c.data.lastSource=null;if(c.data.layerMode==='current')offset.layer=state.activeLayer||'0';state.entities.push(offset);c.data.lastCreatedId=offset.id;
      if(c.data.erase){c.data.lastSource=cloneEntity(entity,false);state.entities=state.entities.filter(x=>x.id!==entity.id);}
      afterGeometryChange();if(offset.approximated)toast(FR?'Décalage créé · courbes de polyligne approximées.':'Offset created · curved polyline approximated.');else toast(CMDT.offsetDone);
      c.data.entityId=null;c.step='selectEntity';setCommandPrompt(FR?`${c.name} Sélectionnez un autre objet ou [Quitter/Annuler] <Quitter>:`:`${c.name} Select another object or [Exit/Undo] <Exit>:`);return;
    }
  }
}
...
function commandNeedsPoint(c){
  ...
  if(c.name==='MTEXT')return ['firstCorner','oppositeCorner'].includes(c.step);
  ...
  if(c.name==='SCALE')return ['base','factor','referenceLength','referenceSecond','newLength','newLengthPoint1','newLengthPointSecond'].includes(c.step);
...
function commandReferencePoint(){
  ...
  if(c.name==='MTEXT'&&c.data.firstCorner)return c.data.firstCorner;
  if(c.name==='ROTATE'){
  ...
  if(c.name==='SCALE'){if(c.step==='referenceSecond'&&c.data.refP1)return c.data.refP1;if(c.step==='newLengthPointSecond'&&c.data.newLengthP1)return c.data.newLengthP1;if(c.data.base)return c.data.base;}
  if(['MOVE','COPY','STRETCH'].includes(c.name)&&c.data.base)return c.data.base;
...
function finishCommand(log=true){
  ...
  state.command=null;state.snapOverride=null;state.trackingPoint=null;state.trackingPoints=[];state.trackingGuides=[];E.workspace.classList.remove('command-active');
...
function cancelCommand(log=true){
  ...
  state.command=null;state.snapOverride=null;state.trackingPoint=null;state.trackingPoints=[];state.trackingGuides=[];state.selectionBox=null;E.workspace.classList.remove('command-active');
...
// V6.9 — AutoCAD-first adaptive dimension engine.
// Dimensions remain export-safe primitives, but are semantic grouped objects in
// Navo2D: draggable dimension/text grips, filled arrowheads, Auto/In/Out arrow
// placement, angular radius dragging and radius/diameter dogleg leaders.
function dimensionSourceBounds(){
  let minX=Infinity,minY=Infinity,maxX=-Infinity,maxY=-Infinity;
  for(const e of state.entities){
    if(e.dimGenerated||e.layer==='DIMENSIONS'||!layerVisible(e.layer))continue;
    for(const q of entitySamples(e)){if(!Number.isFinite(q?.x)||!Number.isFinite(q?.y))continue;minX=Math.min(minX,q.x);minY=Math.min(minY,q.y);maxX=Math.max(maxX,q.x);maxY=Math.max(maxY,q.y);}
  }
...
function dimensionStyle(meta=null){
  const b=dimensionSourceBounds(),diag=b?Math.hypot(b.width,b.height):100,scale=Math.max(diag,1e-6),textScale=Math.max(.25,Number(meta?.textScale)||1),arrowScale=Math.max(.2,Number(meta?.arrowScale)||1);
  const textHeight=Math.max(scale*(Number(state.dimTextRatio)||.02)*textScale,1e-6),arrow=textHeight*(Number(state.dimArrowFactor)||.55)*arrowScale;
  return{textHeight,arrow,gap:textHeight*.36,extGap:textHeight*.18,extBeyond:textHeight*.48,landing:textHeight*(Number(state.dimLeaderLandingFactor)||1.55)};
}
function dimensionBase(type,groupId='preview'){return{id:`dim-${groupId}`,type,rawType:type,layer:'DIMENSIONS',colorMode:'bylayer',colorIndex:256,trueColor:null,approx:false,dimGenerated:true,dimGroupId:groupId};}
function dimLine(a,b,groupId='preview'){if(!a||!b||dist(a,b)<1e-12)return null;return{...dimensionBase('LINE',groupId),p1:copyPoint(a),p2:copyPoint(b)};}
function dimArc(center,radius,start,end,groupId='preview'){if(!(radius>1e-12))return null;return{...dimensionBase('ARC',groupId),center:copyPoint(center),radius,start,end};}
function dimensionTextWidth(value,style){return Math.max(style.textHeight*.4,String(value).length*style.textHeight*.62);}
function dimText(text,center,rotationRad,style,groupId='preview'){
  const value=String(text),h=style.textHeight,a=readableDimensionAngle(rotationRad),ux={x:Math.cos(a),y:Math.sin(a)},uy={x:-Math.sin(a),y:Math.cos(a)},w=dimensionTextWidth(value,style);
  const point={x:center.x-ux.x*w/2+uy.x*h*.62,y:center.y-ux.y*w/2+uy.y*h*.62};return{...dimensionBase('TEXT',groupId),point,text:value,height:h,rotation:a*180/Math.PI};
}
function readableDimensionAngle(a){a=normalizeAngle(a);if(a>Math.PI/2||a<-Math.PI/2)a=normalizeAngle(a+Math.PI);return a;}
function dimArrow(items,tip,pointingDir,size,groupId='preview'){
  const L=Math.hypot(pointingDir.x,pointingDir.y);if(L<1e-12)return;const u={x:pointingDir.x/L,y:pointingDir.y/L},n={x:-u.y,y:u.x},back={x:tip.x-u.x*size,y:tip.y-u.y*size},wing=size*.36;
  items.push({...dimensionBase('POLYLINE',groupId),points:[{x:tip.x,y:tip.y,bulge:0},{x:back.x+n.x*wing,y:back.y+n.y*wing,bulge:0},{x:back.x-n.x*wing,y:back.y-n.y*wing,bulge:0}],closed:true,dimFilled:true});
}
function dimExtension(items,origin,dimPoint,dir,style,groupId='preview'){const L=Math.hypot(dir.x,dir.y);if(L<1e-12)return;const u={x:dir.x/L,y:dir.y/L},a={x:origin.x+u.x*style.extGap,y:origin.y+u.y*style.extGap},b={x:dimPoint.x+u.x*style.extBeyond,y:dimPoint.y+u.y*style.extBeyond},l=dimLine(a,b,groupId);if(l)items.push(l);}
...
function buildCenterMarkDimension(e,groupId='preview',meta=null){
  ...
  const c=e.center,r=e.radius,style=dimensionStyle(meta),gap=Math.max(r*.045,style.textHeight*.18),ext=Math.max(r*.035,style.textHeight*.45),outer=r+ext,items=[];
  const addAxis=(u)=>{
    for(const sign of [-1,1]){
      const a={x:c.x+u.x*gap*sign,y:c.y+u.y*gap*sign},b={x:c.x+u.x*outer*sign,y:c.y+u.y*outer*sign};
      const line=dimLine(a,b,groupId);if(line){line.centerMark=true;line.centerMarkSource=e.id;items.push(line);}
    }
  };
...
function buildLinearDimension(p1,p2,location,orientation='auto',groupId='preview',meta=null){
  ...
  const items=[],u0={x:q2.x-q1.x,y:q2.y-q1.y},L=Math.hypot(u0.x,u0.y);if(L<1e-12)return[];const u={x:u0.x/L,y:u0.y/L},value=dimensionNumber(measure,meta);dimExtension(items,p1,q1,extDir,style,groupId);dimExtension(items,p2,q2,extDir,style,groupId);addDimensionLineWithArrows(items,q1,q2,u,value,style,meta,groupId);items.push(dimText(value,{x:(q1.x+q2.x)/2,y:(q1.y+q2.y)/2},rotation,style,groupId));return items;
}
...
function commitDimension(items,meta=null){
  const valid=(items||[]).filter(Boolean);if(!valid.length)return commandError(FR?'Impossible de créer cette cote.':'Unable to create this dimension.');pushHistory();ensureLayerDefinition('DIMENSIONS');const gid=`D${Date.now().toString(36)}-${state.entitySeq+1}`,m=meta?JSON.parse(JSON.stringify(meta)):null;
...
function scaleEntity(e,base,factor){
  ...
  if(e.type==='TEXT'){e.height*=Math.abs(factor);if(e.mtext)e.width=Math.max(1e-9,Math.abs(Number(e.width)||0)*Math.abs(factor));}
}
...
function toggleDraftSetting(which){
  ...
  else if(which==='otrack'){state.otrack=!state.otrack;if(!state.otrack){state.trackingPoint=null;state.trackingPoints=[];state.trackingGuides=[];}}
...
function trackableSnapType(type){return ['end','mid','center','quad','intersection','apparent','node','gcenter','insert','perp'].includes(type);}
function acquireTrackingPoint(snap){
  if(!state.otrack||!snap||!trackableSnapType(snap.type)||(!state.gripEdit&&(!state.command||!commandNeedsPoint(state.command))))return;
  const pt=copyPoint(snap.point),key=`${pt.x.toFixed(7)},${pt.y.toFixed(7)}`;
  const existing=state.trackingPoints.find(x=>x.key===key);
  if(existing){existing.at=performance.now();return;}
  let direction=null;
  const e=snap.entity;
  if(e?.type==='LINE'){
    const dx=e.p2.x-e.p1.x,dy=e.p2.y-e.p1.y,L=Math.hypot(dx,dy);if(L>1e-12)direction={x:dx/L,y:dy/L};
  }
  state.trackingPoints.push({key,point:pt,type:snap.type,entityId:e?.id||null,direction,at:performance.now()});
  if(state.trackingPoints.length>8)state.trackingPoints.splice(0,state.trackingPoints.length-8);
  state.trackingPoint=pt;
}
function trackingLineExtent(point,dir){
  const span=Math.max((state.bounds?.width||100),(state.bounds?.height||100),100/state.view.scale)*30;
  return{from:{x:point.x-dir.x*span,y:point.y-dir.y*span},to:{x:point.x+dir.x*span,y:point.y+dir.y*span}};
}
function applyObjectTracking(raw){
  state.trackingGuides=[];state.trackingActive=false;
  if(!state.otrack||!state.trackingPoints.length)return raw;
  const tol=9/state.view.scale,candidates=[];
  const addCandidate=(point,source,dir,kind,score)=>{
    if(!point||!Number.isFinite(point.x)||!Number.isFinite(point.y)||score>tol)return;
    candidates.push({point,source,dir,kind,score});
  };
  for(const tp of state.trackingPoints){
    addCandidate({x:tp.point.x,y:raw.y},tp,{x:0,y:1},'vertical',Math.abs(raw.x-tp.point.x));
    addCandidate({x:raw.x,y:tp.point.y},tp,{x:1,y:0},'horizontal',Math.abs(raw.y-tp.point.y));
    if(state.polar){
      const inc=Math.max(.1,Number(state.polarIncrement)||45)*Math.PI/180;
      for(let a=0;a<Math.PI-1e-8;a+=inc){
        const dir={x:Math.cos(a),y:Math.sin(a)},vx=raw.x-tp.point.x,vy=raw.y-tp.point.y,t=vx*dir.x+vy*dir.y;
        const proj={x:tp.point.x+dir.x*t,y:tp.point.y+dir.y*t},d=dist(raw,proj);
        addCandidate(proj,tp,dir,'polarTrack',d);
      }
    }
    if(tp.direction){
      const n={x:-tp.direction.y,y:tp.direction.x},vx=raw.x-tp.point.x,vy=raw.y-tp.point.y,t=vx*n.x+vy*n.y,proj={x:tp.point.x+n.x*t,y:tp.point.y+n.y*t};
      addCandidate(proj,tp,n,'perpendicular',dist(raw,proj));
    }
  }
  // Virtual intersection of one acquired X alignment and another acquired Y alignment.
  let bestCross=null;
  for(let i=0;i<state.trackingPoints.length;i++)for(let j=0;j<state.trackingPoints.length;j++){
    if(i===j)continue;const a=state.trackingPoints[i],b=state.trackingPoints[j],p={x:a.point.x,y:b.point.y},d=dist(raw,p);
    if(d<=tol*1.35&&(!bestCross||d<bestCross.score))bestCross={point:p,a,b,score:d};
  }
  if(bestCross){
    const gv=trackingLineExtent(bestCross.a.point,{x:0,y:1}),gh=trackingLineExtent(bestCross.b.point,{x:1,y:0});
    state.trackingGuides=[{...gv,type:'track'},{...gh,type:'track'}];state.trackingActive=true;return bestCross.point;
  }
  candidates.sort((a,b)=>a.score-b.score);
  const best=candidates[0];if(!best)return raw;
  const ext=trackingLineExtent(best.source.point,best.dir);state.trackingGuides=[{...ext,type:'track',kind:best.kind}];state.trackingActive=true;
  return best.point;
}

function resolveDraftPoint(clientX,clientY,ignoreSnap=false){
  let raw=screenToWorld(clientX,clientY);state.draftingGuide=null;state.trackingGuides=[];state.trackingActive=false;
  if(state.gridSnap){const step=Math.max(1e-12,state.gridSnapStep);raw={x:Math.round(raw.x/step)*step,y:Math.round(raw.y/step)*step};}
  const ref=commandReferencePoint();
  ...
  if(state.otrack&&state.trackingPoints.length)raw=applyObjectTracking(raw);

  if(ref&&(state.snapOverride==='parallel'||state.osnapModes.has('parallel'))&&!state.ortho&&!state.trackingActive){const pg=parallelGuidePoint(ref,raw);if(pg){raw=pg.point;state.draftingGuide={from:ref,to:raw,type:'parallel'};}}
  if(ref&&state.ortho&&!state.trackingActive){
  ...
  }else if(ref&&state.polar&&!state.trackingActive&&state.draftingGuide?.type!=='parallel'){
...
function nearestSnap(clientX,clientY,reference=null){
  ...
  if(state.snapHoverKey===key){if(now-(state.snapHoverSince||now)>320)acquireTrackingPoint(best);}else{state.snapHoverKey=key;state.snapHoverSince=now;}
  return best;
}
...
function drawDraftingGuides(){
  if(state.otrack&&state.trackingGuides?.length){
    ctx.save();ctx.strokeStyle='rgba(53,211,154,.82)';ctx.fillStyle='#35d39a';ctx.lineWidth=1;ctx.setLineDash([5,4]);
    for(const guide of state.trackingGuides){const a=worldToScreen(guide.from),b=worldToScreen(guide.to);ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();}
    ctx.setLineDash([]);
    for(const tp of state.trackingPoints||[]){const q=worldToScreen(tp.point);ctx.beginPath();ctx.moveTo(q.x-4,q.y);ctx.lineTo(q.x+4,q.y);ctx.moveTo(q.x,q.y-4);ctx.lineTo(q.x,q.y+4);ctx.stroke();}
    ctx.restore();
  }
...
function previewLengthenEntity(e,pick,delta){
  if(!e||!Number.isFinite(Number(delta)))return null;const n=cloneEntity(e,false),d=Number(delta);
  if(n.type==='LINE'){
    const first=dist(pick,n.p1)<dist(pick,n.p2),target=first?n.p1:n.p2,other=first?n.p2:n.p1,v={x:target.x-other.x,y:target.y-other.y},L=Math.hypot(v.x,v.y);
    if(L<1e-12||L+d<=1e-9)return null;const k=(L+d)/L,q={x:other.x+v.x*k,y:other.y+v.y*k};if(first)n.p1=q;else n.p2=q;return n;
  }
  if(n.type==='ARC'){
    const a={x:n.center.x+n.radius*Math.cos(n.start),y:n.center.y+n.radius*Math.sin(n.start)},b={x:n.center.x+n.radius*Math.cos(n.end),y:n.center.y+n.radius*Math.sin(n.end)},da=d/n.radius;
    if(dist(pick,a)<dist(pick,b))n.start-=da;else n.end+=da;return n;
  }
  return null;
}
function breakPreviewEntities(e,p1,p2){
  if(!e||e.type!=='LINE'||!p1||!p2)return[];const q1=projectPointToLine(p1,e.p1,e.p2,true),q2=projectPointToLine(p2,e.p1,e.p2,true),t1=lineParam(q1,e.p1,e.p2),t2=lineParam(q2,e.p1,e.p2),lo=Math.min(t1,t2),hi=Math.max(t1,t2),out=[];
  if(lo>1e-8){const a=cloneEntity(e,false);a.p1=copyPoint(e.p1);a.p2={x:e.p1.x+(e.p2.x-e.p1.x)*lo,y:e.p1.y+(e.p2.y-e.p1.y)*lo};out.push(a);}if(hi<1-1e-8){const b=cloneEntity(e,false);b.p1={x:e.p1.x+(e.p2.x-e.p1.x)*hi,y:e.p1.y+(e.p2.y-e.p1.y)*hi};b.p2=copyPoint(e.p2);out.push(b);}return out;
}
function trimLineRemovalPreview(entity,pick){
  if(entity?.type!=='LINE')return null;const ts=uniqueNumbers(intersectionsWithEntity(entity).map(p=>lineParam(p,entity.p1,entity.p2)).filter(t=>t>1e-8&&t<1-1e-8));
  if(!ts.length)return cloneEntity(entity,false);const projected=projectPointToLine(pick,entity.p1,entity.p2,true)||pick,tc=Math.max(0,Math.min(1,lineParam(projected,entity.p1,entity.p2))),cuts=[0,...ts,1];let k=0;for(let i=0;i<cuts.length-1;i++){if(tc<cuts[i+1]-1e-10||i===cuts.length-2){k=i;break;}}const lo=cuts[k],hi=cuts[k+1],v={x:entity.p2.x-entity.p1.x,y:entity.p2.y-entity.p1.y},pt=t=>({x:entity.p1.x+v.x*t,y:entity.p1.y+v.y*t}),n=cloneEntity(entity,false);n.p1=pt(lo);n.p2=pt(hi);return n;
}
function previewExtendEntity(entity,pick){
  if(!entity)return null;
  if(entity.type==='LINE'){
    const moveP1=dist(pick,entity.p1)<dist(pick,entity.p2),origin=moveP1?entity.p2:entity.p1,current=moveP1?entity.p1:entity.p2,v={x:current.x-origin.x,y:current.y-origin.y},L=Math.hypot(v.x,v.y);if(L<1e-12)return null;const dir={x:v.x/L,y:v.y/L},hits=[];for(const o of state.entities)if(o.id!==entity.id&&layerVisible(o.layer))hits.push(...rayIntersectionsWithEntity(origin,dir,o).filter(h=>h.t>L+1e-8));if(!hits.length)return null;hits.sort((a,b)=>a.t-b.t);const n=cloneEntity(entity,false);if(moveP1)n.p1=copyPoint(hits[0].p);else n.p2=copyPoint(hits[0].p);return n;
  }
  if(entity.type==='POLYLINE'&&!entity.closed&&entity.points.length>=2){const n=cloneEntity(entity,false),first=n.points[0],last=n.points[n.points.length-1],moveFirst=dist(pick,first)<dist(pick,last),origin=moveFirst?n.points[1]:n.points[n.points.length-2],current=moveFirst?first:last,v={x:current.x-origin.x,y:current.y-origin.y},L=Math.hypot(v.x,v.y);if(L<1e-12)return null;const dir={x:v.x/L,y:v.y/L},hits=[];for(const o of state.entities)if(o.id!==entity.id&&layerVisible(o.layer))hits.push(...rayIntersectionsWithEntity(origin,dir,o).filter(h=>h.t>L+1e-8));if(!hits.length)return null;hits.sort((a,b)=>a.t-b.t);if(moveFirst)n.points[0]=copyPoint(hits[0].p);else n.points[n.points.length-1]=copyPoint(hits[0].p);return n;}
  return null;
}
function filletPreviewEntities(a,b,pickA,pickB,r,trim=true){
  if(a?.type!=='LINE'||b?.type!=='LINE')return[];const radius=Number(r),I=infiniteLineIntersection(a.p1,a.p2,b.p1,b.p2);if(!I||!(radius>=0))return[];const d1=branchDirection(a,I,pickA),d2=branchDirection(b,I,pickB);if(!d1||!d2)return[];const dot=Math.max(-1,Math.min(1,d1.x*d2.x+d1.y*d2.y)),theta=Math.acos(dot);if(theta<1e-7||Math.abs(Math.PI-theta)<1e-7)return[];
  const la=cloneEntity(a,false),lb=cloneEntity(b,false);if(radius<=1e-12){if(trim){trimLineToPoint(la,I,d1,I);trimLineToPoint(lb,I,d2,I);return[la,lb];}return[];}
  const t=radius/Math.tan(theta/2),p1={x:I.x+d1.x*t,y:I.y+d1.y*t},p2={x:I.x+d2.x*t,y:I.y+d2.y*t},bis={x:d1.x+d2.x,y:d1.y+d2.y},bl=Math.hypot(bis.x,bis.y);if(bl<1e-12)return[];bis.x/=bl;bis.y/=bl;const cd=radius/Math.sin(theta/2),center={x:I.x+bis.x*cd,y:I.y+bis.y*cd};let sA=Math.atan2(p1.y-center.y,p1.x-center.x),sB=Math.atan2(p2.y-center.y,p2.x-center.x);if(positiveSpan(sA,sB)>Math.PI){const z=sA;sA=sB;sB=z;}if(trim){trimLineToPoint(la,I,d1,p1);trimLineToPoint(lb,I,d2,p2);}const arc={...cloneEntity(a,false),type:'ARC',rawType:'ARC',center,radius,start:sA,end:sB};delete arc.p1;delete arc.p2;return trim?[la,lb,arc]:[arc];
}
function chamferPreviewEntities(a,b,pickA,pickB,dA,dB,trim=true){
  if(a?.type!=='LINE'||b?.type!=='LINE')return[];const x=Number(dA),y=Number(dB),I=infiniteLineIntersection(a.p1,a.p2,b.p1,b.p2);if(!I||x<0||y<0)return[];const u=branchDirection(a,I,pickA),v=branchDirection(b,I,pickB);if(!u||!v)return[];const p1={x:I.x+u.x*x,y:I.y+u.y*x},p2={x:I.x+v.x*y,y:I.y+v.y*y},la=cloneEntity(a,false),lb=cloneEntity(b,false);if(trim){trimLineToPoint(la,I,u,p1);trimLineToPoint(lb,I,v,p2);}const out=trim?[la,lb]:[];if(dist(p1,p2)>1e-10){const n=cloneEntity(a,false);n.p1=p1;n.p2=p2;out.push(n);}return out;
}
function arraySelectionBounds(){const pts=[];for(const e of selectedEntities())pts.push(...selectionSamples(e));return boundsFromPoints(pts);}
function rectArrayPreviewEntities(c){
  const src=selectedEntities();if(!src.length)return[];const b=arraySelectionBounds(),cols=Math.max(1,Math.min(20,Math.round(c.data.columns||2))),rows=Math.max(1,Math.min(20,Math.round(c.data.rows||2))),typed=parseNumber(E.commandInput?.value||'');
  let sx=Number(c.data.colSpacing),sy=Number(c.data.rowSpacing);if(!Number.isFinite(sx))sx=Math.max((b?.width||10)*1.25,1);if(c.step==='colSpacing'&&Number.isFinite(typed))sx=typed;if(!Number.isFinite(sy))sy=Math.max((b?.height||10)*1.25,1);if(c.step==='rowSpacing'&&Number.isFinite(typed))sy=typed;
  const out=[];for(let r=0;r<rows;r++)for(let col=0;col<cols;col++){if(r===0&&col===0)continue;for(const e of src){if(out.length>180)return out;const n=cloneEntity(e,false);translateEntity(n,col*sx,r*sy);out.push(n);}}return out;
}
function polarArrayPreviewEntities(c){
  const src=selectedEntities(),center=c.data.center;if(!src.length||!center)return[];const typed=parseNumber(E.commandInput?.value||''),items=Math.max(2,Math.min(48,Math.round(c.data.items||4))),fillDeg=c.step==='fillAngle'&&Number.isFinite(typed)?typed:(c.data.fillAngle??360),fill=fillDeg*Math.PI/180,full=Math.abs(Math.abs(fill)-Math.PI*2)<1e-8,step=full?fill/items:fill/(items-1),out=[];
  for(let i=1;i<items;i++)for(const e of src){if(out.length>180)return out;const n=cloneEntity(e,false);transformEntityPoints(n,p=>rotatePoint(p,center,step*i));out.push(n);}return out;
}
...
function drawCommandPreview(){
  ...
  else if(c.name==='MTEXT'&&c.step==='oppositeCorner'&&c.data.firstCorner){
    const box=mtextBoxFromCorners(c.data.firstCorner,p,c.data),a=box.origin,b={x:box.right,y:box.origin.y},d={x:box.origin.x,y:box.bottom},cc={x:box.right,y:box.bottom};
    ctx.save();ctx.setLineDash([4,3]);ctx.strokeStyle='#d9e2e7';line(a,b);line(b,cc);line(cc,d);line(d,a);ctx.restore();
    const sp=worldToScreen(cc);ctx.save();ctx.font='800 10px ui-monospace,monospace';ctx.fillStyle='#d7e6ee';ctx.fillText(`${fmt(box.width)} × ${fmt(box.boxHeight)}`,sp.x+8,sp.y+16);ctx.restore();
  }
  ...
  else if(c.name==='ARRAYRECT'){drawPreviewEntities(rectArrayPreviewEntities(c));}
  else if(c.name==='ARRAYPOLAR'&&c.data.center){drawPreviewEntities(polarArrayPreviewEntities(c));}
  ...
  else if(c.name==='TRIM'&&c.step==='pick'){const hit=hitTest(state.pointer.x,state.pointer.y),pick=screenToWorld(state.pointer.x,state.pointer.y),removed=hit?trimLineRemovalPreview(hit,pick):null;if(removed){ctx.save();ctx.strokeStyle='#ff7d7d';ctx.fillStyle='#ff7d7d';ctx.setLineDash([5,3]);drawEntity(removed,'#ff7d7d',2.4);ctx.restore();}}
  else if(c.name==='EXTEND'&&c.step==='pick'){const hit=hitTest(state.pointer.x,state.pointer.y),pick=screenToWorld(state.pointer.x,state.pointer.y),extended=hit?previewExtendEntity(hit,pick):null;if(extended)drawEntity(extended,'#7bc4ff',1.8);}
  ...
  else if(c.name==='SCALE'&&c.data.base&&['factor','referenceSecond','newLength','newLengthPointSecond'].includes(c.step)){
    let factor=null,guideFrom=c.data.base;
    if(c.step==='factor'){const typed=parseNumber(E.commandInput?.value||'');factor=(typed>0)?typed:dist(c.data.base,p);}
    else if(c.step==='referenceSecond'&&c.data.refP1){guideFrom=c.data.refP1;line(guideFrom,p);}
    else if(c.step==='newLength'){const typed=parseNumber(E.commandInput?.value||''),ref=Number(c.data.referenceLength)||1,n=(typed>0)?typed:dist(c.data.base,p);factor=n/ref;}
    else if(c.step==='newLengthPointSecond'&&c.data.newLengthP1){guideFrom=c.data.newLengthP1;const ref=Number(c.data.referenceLength)||1;factor=dist(guideFrom,p)/ref;}
    if(Number.isFinite(factor)&&factor>1e-9){line(guideFrom,p);drawSelectedPreview(e=>{const n=cloneEntity(e,false);scaleEntity(n,c.data.base,factor);return n;},false);}
  }
  ...
  else if(c.name==='LENGTHEN'&&c.step==='selectEntity'){const hit=hitTest(state.pointer.x,state.pointer.y),q=hit?previewLengthenEntity(hit,screenToWorld(state.pointer.x,state.pointer.y),c.data.delta):null;if(q)drawEntity(q,'#7bc4ff',1.8);}
  else if(c.name==='BREAK'&&c.step==='second'&&c.data.entityId&&c.data.p1){const e=state.entities.find(x=>x.id===c.data.entityId);drawPreviewEntities(breakPreviewEntities(e,c.data.p1,p));}
  else if((c.name==='FILLET'||c.name==='CHAMFER')&&c.step==='second'&&c.data.firstId){const first=state.entities.find(e=>e.id===c.data.firstId),second=hitTest(state.pointer.x,state.pointer.y),pickB=screenToWorld(state.pointer.x,state.pointer.y);if(first&&second&&second.id!==first.id&&second.type==='LINE'){if(c.name==='FILLET')drawPreviewEntities(filletPreviewEntities(first,second,c.data.firstPick,pickB,c.data.radius??state.filletRadius,c.data.trim!==false));else{let d2=c.data.b??state.chamferB;if((c.data.method??state.chamferMethod)==='angle')d2=chamferSecondDistanceByAngle(first,second,c.data.firstPick,pickB,c.data.angleDistance??state.chamferAngleDistance,c.data.angle??state.chamferAngle);if(d2!=null)drawPreviewEntities(chamferPreviewEntities(first,second,c.data.firstPick,pickB,(c.data.method??state.chamferMethod)==='angle'?(c.data.angleDistance??state.chamferAngleDistance):(c.data.a??state.chamferA),d2,c.data.trim!==false));}}}
  ctx.restore();
