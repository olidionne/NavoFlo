(() => {
  if (window.__NavoFloLeaseBootstrap) return;
  window.__NavoFloLeaseBootstrap = true;
  const script = document.currentScript;
  const product = String(script?.dataset?.product || '').toLowerCase();
  const modulePath = String(script?.dataset?.module || '');
  if (!product) return;
  const appName = product.toUpperCase();
  ...
  function accountUrl(){ return fr?'/account/licenses/':'/en/account/licenses/'; }
  ...
  async function heartbeat(){
    ...
    try{
      ...
      lockLease(fr?`Cette licence vient d’être transférée ou reprise sur un autre poste. ${appName} a été verrouillé sur ce poste.`:`This license was just transferred or taken over on another device. ${appName} has been locked on this device.`,{moved:e.code==='LEASE_INVALID'});
    }
  }
  ...
  async function startApp(){
    ...
    if(modulePath){
      ...
      catch(e){ console.error(e); card(`<div class="nlg-kicker">NAVOFLO</div><h2>${fr?`Impossible de démarrer ${appName}`:`Unable to start ${appName}`}</h2><p>${String(e?.message||e)}</p><div class="nlg-actions"><button class="primary" onclick="location.reload()">${fr?'Recharger':'Reload'}</button></div>`); }
    }
  }
