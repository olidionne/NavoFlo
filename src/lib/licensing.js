...
export async function acquireAppLease(request,env,context,payload={}){
  ...
  // Multiple applications/tabs on the SAME workstation are allowed.
  // Each gets its own short-lived token. A license conflict exists only when a
  // different device tries to use the same seat. This lets Navo2D and Navo3D
  // remain open together on one workstation without consuming another seat.
